...
- `GET /users/me/student-wallet-sepay-static-qr` — trả **QR SePay tĩnh** cho học sinh hiện tại; QR không chứa số tiền, nội dung chuyển khoản là `NAPVI <student_info.id> <active_class_id...>` và response có `classIds`. Frontend hiển thị QR này trực tiếp trong popup nạp ví; webhook mới cộng `account_balance`.
