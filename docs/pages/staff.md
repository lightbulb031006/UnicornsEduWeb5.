...
- `/staff/classes/[id]`
  ...
  - card **Lịch dạy bù** xuất hiện ngay trên khu vực lịch sử buổi học; danh sách lấy theo **tháng đang chọn** của khu vực lịch sử/khảo sát, sắp theo buổi gần nhất trước và có phân trang ngay trong card; đổi tháng reset pagination và fetch đúng khoảng ngày của tháng đó
...
- **Backend routes đang dùng**
  ...
  - `GET /student/:id/wallet-sepay-static-qr`
    - `staff.assistant` và `staff.accountant` xem QR SePay tĩnh cho học sinh xem được; `staff.customer_care` chỉ xem được khi học sinh đang thuộc CSKH của chính staff hiện tại. QR note là `[SEPAY_TRANSFER_NOTE_PREFIX] UNIST-{uuid} UNICL-{uuid}... LOP <tên lớp...>` và không chứa số tiền; webhook reconcile theo student/class id ở đầu nội dung và vẫn tương thích marker `NAPVI` cũ. Không role staff nào được gọi `PATCH /student/update-student-account-balance` để chỉnh thẳng số dư.
